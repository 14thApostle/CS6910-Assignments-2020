from __future__ import print_function
import gensim.downloader as api # package to download text corpus
import nltk # text processing
from nltk.corpus import stopwords
import string
import numpy as np
import pickle
import os

import torch
from torch import nn
from torch.autograd import Variable
from torch.optim import SGD
import torch.nn.functional as F
from torch.utils.data import DataLoader, TensorDataset

import csv
import pandas as pd
import matplotlib.pyplot as plt

# download stopwords
nltk.download('stopwords')

# download textcorpus
data = api.load('text8')

# collect all words to be removed
stop = stopwords.words('english') + list(string.punctuation)

actual_words = []
cleaned_words = []
unique_words = set()

# remove stop words
print('removing stop words from text corpus')
for i,words in enumerate(data):
    current_nonstop_words = [w for w in words if w not in stop]
    cleaned_words += current_nonstop_words
    actual_words += words

    for ns in current_nonstop_words:
        unique_words.add(ns)

# print statistics
print(len(actual_words), 'words BEFORE cleaning stop words and punctuations')
print(len(cleaned_words), 'words AFTER cleaning stop words and punctuations')
print('vocabulary size: ', len(unique_words))

# 'cleaned_words' and 'unique_words' to create a word2vec model

vocab = list(unique_words)
print('vocabulary size: ', len(vocab))

WINDOW_SIZE = 4 ## left and right each
EMBEDDING_SIZE = 300
EPOCH = 100
BATCH_SIZE = 1024

class CBOW(nn.Module):
    def __init__(self, vocab_size, embedding_size, window_size):
        super(CBOW, self).__init__()

        self.vocab_size = vocab_size
        self.embedding_size = embedding_size
        self.window_size = window_size
        self.embeddings = nn.Embedding(self.vocab_size, self.embedding_size)

        self.linear1 = nn.Linear(self.embedding_size, 128)
        self.activation_function1 = nn.ReLU()
        
        self.linear2 = nn.Linear(128, self.vocab_size)
        self.activation_function2 = nn.LogSoftmax(dim = -1)

    def forward(self, inputs):
        embeds = self.embeddings(inputs)
        embeds = torch.sum(embeds,dim=[1])
        out = self.linear1(embeds)
        out = self.activation_function1(out)
        out = self.linear2(out)
        out = self.activation_function2(out)
        return out
    
    def get_word_vector(self, word_idx):
        word = Variable(torch.LongTensor([word_idx]))
        return self.embeddings(word).view(1, -1)

def make_context_vector(context, word_to_idx):
    idxs = [word_to_idx[w] for w in context]
    return idxs
    # return torch.tensor(idxs, dtype=torch.long)

def train_cbow(train_loader, unique_vocab, word_to_idx):
    cbow = CBOW(len(unique_vocab)+1, EMBEDDING_SIZE, WINDOW_SIZE)
    if torch.cuda.is_available():
        cbow.cuda()
    loss_fn = nn.NLLLoss()  # loss function
    optimizer = SGD(cbow.parameters(), lr=0.1)
    
    for epoch in range(0,EPOCH):
        counter = 0
        total_loss = 0
        for context, target in train_loader: 
            counter += 1 
            inp_var = context.cuda()
            target_var = target.cuda()
                
            cbow.zero_grad()
            
            log_prob = cbow(inp_var)
            loss = loss_fn(log_prob, target_var)
            total_loss += loss.data
            
            loss.backward()
            optimizer.step()

        print("{}/{} loss {:.2f}".format(epoch, EPOCH, total_loss/counter))

        if epoch%10 == 0:
            PATH = "./cbow_model_final_e{}.pth".format(epoch)
            torch.save(cbow.state_dict(), PATH)
    return cbow

text = list(cleaned_words)

# mapping to index
word_to_idx = {w: i+1 for i, w in enumerate(vocab)}
idx_to_word = {ix+1:word for ix, word in enumerate(vocab)}

# save the dict so that we can load corresponding embeddings in the lstm
with open('cbow_w2i.pickle', 'wb') as handle:
    pickle.dump(word_to_idx, handle, protocol=pickle.HIGHEST_PROTOCOL)
with open('cbow_i2w.pickle', 'wb') as handle:
    pickle.dump(idx_to_word, handle, protocol=pickle.HIGHEST_PROTOCOL)
    
train_x = []
train_y = []
for i in range(WINDOW_SIZE, len(text) - WINDOW_SIZE):
    data_context = text[i-WINDOW_SIZE:i] + text[i+1:i+1+WINDOW_SIZE] 
    data_target = text[i]
    train_x.append( make_context_vector(data_context, word_to_idx) )
    train_y.append(word_to_idx[data_target])

train_x = torch.tensor(train_x, dtype=torch.long)
train_y = torch.tensor(train_y, dtype=torch.long)

train_data = TensorDataset(train_x, train_y)
train_loader = DataLoader(train_data, shuffle=False, batch_size=BATCH_SIZE, drop_last=True)

# train model
cbow = train_cbow(train_loader, vocab, word_to_idx)

PATH = "./cbow_model_final.pth"
torch.save(cbow.state_dict(), PATH)

embeddings = cbow.embeddings.weight.data.cpu()
np.savez('cbow_embeddings.npz', name1=embeddings, name2=np.array(list(word_to_idx.keys())))

